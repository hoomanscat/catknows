...
from skoolhud.ai.tools import discord_report_post
from skoolhud.ai.tools import STATUS_DIR
import json
import time
import os
...
def main():
    ...
    try:
        ...
        try:
            ...
            cols = [r[1] for r in s.execute(text("PRAGMA table_info(members)"))]
    ...
    # Optionally post weekly joiners to Discord
    webhook = os.getenv('DISCORD_WEBHOOK_JOINERS')
    if webhook and (out_dir / 'new_joiners_week.md').exists():
        from skoolhud.ai.tools import post_guard_allowed, post_guard_mark
        if post_guard_allowed(args.slug, 'joiners', cooldown_env_var='JOINERS_POST_COOLDOWN'):
            status = discord_report_post(str(webhook), out_dir / 'new_joiners_week.md', username=f"Joiners ({args.slug})")
            if status:
                post_guard_mark(args.slug, 'joiners')
            print(f"Discord post status (joiners): {status}")
        else:
            print("Skipping Discord post: Joiners cooldown not expired")
